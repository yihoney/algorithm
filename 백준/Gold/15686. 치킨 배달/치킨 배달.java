...
public class Main {
    static List<Point> chickenArr; // 치킨집들의 위치를 저장할 배열
    static List<Point> homeArr; // 집들의 위치를 저장할 배열
    static int[] curChickenIdxArr; // 조합 시 뽑은 치킨집을 저장할 배열
    ...
    public static void main(String[] args) throws IOException {
        ...
        M = Integer.parseInt(st.nextToken()); // 폐업시키지 않을 치킨집의 최대 갯수

        chickenArr = new ArrayList<Point>(13); // 치킨집의 개수는 최대 13이라고 주어짐
        homeArr = new ArrayList<Point>(2 * N); // 집의 개수는 2N개를 넘지 않는다고 주어짐
        ...
        curChickenIdxArr = new int[M]; // M개의 치킨집을 가지므로 크기가 M인 배열 선언
        getCombiChickenArr(0, 0); // 조합 생성 메서드 호출
    ...
    /**
     * 
     * 입력받은 치킨집들 중 M개의 치킨집을 뽑는 메서드 (조합 수행)
     * 
     * @param cnt      // 현재까지 뽑은 치킨집 갯수
     * @param startIdx // 조합 시도를 할 때 시작할 인덱스 (중복 허용하지 않기 위해 시작 인덱스 기억 목적)
     * @param r        // 뽑을 치킨집 갯수
     */
    private static void getCombiChickenArr(int cnt, int startIdx) {
        if (cnt == M) { // M개의 치킨집을 다 뽑았다면 (조합 생성 완료)
            int curChickenDist = getCityChickenDist(curChickenIdxArr); // M개의 치킨집을 가지는 배열을 매개변수로 넘겨 해당 경우의 도시치킨거리 구함
            cityChickenDist = Math.min(cityChickenDist, curChickenDist); // 현재의 도시치킨거리와 기존의 도시치킨거리를 비교해 최소값 저장
            return;
        }

        for (int i = startIdx; i < chickenArr.size(); i++) {
            curChickenIdxArr[cnt] = i;
            getCombiChickenArr(cnt + 1, i + 1);
        }
     ...
     * @return 도시치킨거리
     */
    private static int getCityChickenDist(int[] subChickenIdxArr) {
        int dist; // 치킨거리
        int curCityChickenDist = 0; // 현재 도시치킨거리
        for (int h = 0; h < homeArr.size(); h++) { // 모든 집의 치킨거리를 구하기 위한 반복문
            int minDist = Integer.MAX_VALUE; // 치킨거리는 집마다 바뀌므로 여기서 초기화! (최소 한 번은 바뀌어야 하므로 가장 큰 값을 초깃값으로 설정)

            for (int c = 0; c < subChickenIdxArr.length; c++) { // 모든 치킨집과 현재 집의 거리를 구하기 위한 반복문
                // 집과 치킨집 사이의 치킨거리: |r1-r2| + |c1-c2|
                Point home = homeArr.get(h);
                Point chicken = chickenArr.get(subChickenIdxArr[c]);
                dist = Math.abs(home.c - chicken.c) + Math.abs(home.r - chicken.r);
