...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int ans = 0; // 미세먼지가 0보다 큰 값들만 더해서 미세먼지의 양을 계산 (-1은 공기청정기이기 때문에 이를 통해 생략 가능)
        for (int i = 1; i <= R; i++) {
            for (int j = 1; j <= C; j++) {
                if (arr2[i][j] > 0) {
                    ans += arr2[i][j];
    ...
    public static void solution() {
        ...
        // 2. 공기청정기 돌리기
        doAirPurifier(air[0][0], air[0][1], leftDir);

        // 아래쪽 시계방향 순환 , 방향벡터 (우, 하, 좌, 상)
        doAirPurifier(air[1][0], air[1][1], rightDir);

        for (int i = 0; i < arr2.length; i++) { // 공기청정기 작동한 결과를 arr에 갱신
     ...
     * 공기청정기 작동 메서드
     * 
     * @param airX      공기청정기 행 위치
     * @param airY      공기청정기 열 위치
     * @param direction 방향벡터 (반시계 or 시계)
     */
    private static void doAirPurifier(int airX, int airY, int[][] direction) {
        ...
        while (true) {
            int nx = cx + direction[d][0];
            int ny = cy + direction[d][1];
            ...
            if (nx == airX && ny == airY) { // 시작점으로(공기청정기가 있는 곳) 돌아오면 반복문 탈출
                break; // 미세먼지 흡수 및 종료
            }
